...
def probe_living_map(res: Result) -> None:
    ...
    a served, auth-gated, read-only node/edge graph whose every status is backed by a real source
    (honest 'unknown' where not instrumented), proven derived + drift-free. Certs:
